...
### QACE-Driven Crypto Risk Controls

- **QACE as a cryptographic risk oracle** – `pqcnet-qace` already ingests latency, loss, chaos, and anomaly counters from telemetry; we now treat those feeds as a crypto risk oracle too. For every tunnel/epoch the engine blends anomaly metrics (retries, replay hints, suspicious route churn), policy switches (e.g., `DRILL_MODE`, regulator scenarios), and external inputs such as the “Kyber under review” toggle from ops into a normalized score. That score flips simple policy bits like `kem_degraded(KYBER)` and `require_backup_kem = true`, making it easy to say “new sessions must favor the backup KEM” without implying a cryptanalytic break—QACE is a risk router, not a magician.
- **HQC as a controlled backup path** – PQCNet relayers, sentries, and DW3B overlays advertise both Kyber and HQC key material while flagging HQC as `backup_only = true`. In steady state Kyber carries the traffic, but whenever the QACE flags above fire—or ops temporarily disables Kyber—new sessions are brought up on HQC instead. Existing Kyber tunnels drain naturally while fresh tunnels inherit the HQC material, so drills and remediation windows never take the mesh offline.
- **Full visibility via telemetry** – `pqcnet-telemetry` grows a per-tunnel record of “which KEM did we use?” (Kyber vs. HQC) and a single-word “why” tag (`normal | drill | fallback`). Chronosync and DW3B regulators get a clean audit trail that shows when HQC handled production load, why it was selected, and how quickly the network pivoted back to Kyber once the drill or incident ended.
- **Redundant KEM + signatures during Kyber remediation** – When Kyber enters remediation, the backup path couples HQC key exchange with the `autheo-pqcnet-sphincs` dual-signature stack. KEMs shift from Kyber→HQC while signatures slide from Dilithium→SPHINCS+, so both layers keep NIST-aligned redundancy without forcing a maintenance window. Critical DW3B and Chronosync flows stay online, but they do so with lattice + hash-based coverage the entire time.
...
- `autheo-dw3b-mesh/` – Dark Web Privacy Network engine that orchestrates routing,
  ...
  caches its params + pinned key metadata under `config/crypto/` and installs a
  single-threaded Rayon pool (override via `RAYON_NUM_THREADS` or
  `AUTHEO_RAYON_THREADS`) so the heavy suite doesn’t try to spawn 128 workers
  per proof.
